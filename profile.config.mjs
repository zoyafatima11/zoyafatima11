// Everything the figures and generated sections are built from.
// The prose (abstract etc.) lives in README.template.md; edit that for wording.
// After changing either file: `npm run build`, or just push and the Action rebuilds.
...
export default {
  ...
  // Contact links: blank ones are hidden. Shown as a "Correspondence" line.
  links: {
    email: "",
    website: "",
    linkedin: "",
  ...
  // "Cite this profile" BibTeX entry.
  bibtex: {
  ...
  // Figure 3 (contribution log) needs a token, so it only renders in the GitHub Action.
  activity: true,
};
